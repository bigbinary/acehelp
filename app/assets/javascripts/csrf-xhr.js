            ...
            if (csrfToken === null) {
                console.warn(
                    "csrf-xhr was unable to add a X-CSRF-Token header to a " +
                        ...
                        " to " +
                        url +
                        ' because when csrf-xhr originally loaded, it could not find a <meta> element in the <head> with name="csrf-token" and a Rails CSRF token in a content attribute.'
                );
            } else {
