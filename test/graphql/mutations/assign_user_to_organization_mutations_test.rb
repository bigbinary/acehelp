...
class Mutations::AssignUserToOrganizationMutationsTest < ActiveSupport::TestCase
  ...
  test "assign organization" do
    result = AceHelp::ClientLoggedIn.call(@ethan).execute(@query, user_keys: { email: @ethan.email })
  ...
  test "api should return new user" do
    ...
    result =  AceHelp::ClientLoggedIn.call(@ethan).execute(@query, user_keys: { email: new_email })
    assert_equal new_email,  result.data.assign_user_to_organization.user.email
  end

  test "assign organization with name" do
    new_email_2 =
    result = AceHelp::ClientLoggedIn.call(@ethan).execute(@query, user_keys: { email: "new_email_2@example.com", firstName: "Sagar", lastName: "Alias Jackey" })
